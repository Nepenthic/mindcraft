(async (bot) => {
    ...
    for (let y = startY - 1; y >= startY - 10; y--) {
        await bot.dig(bot.blockAt(new Vec3(botPosition.x, y, botPosition.z))); if(bot.interrupt_code) {log(bot, "Code interrupted.");return;}
    }
    
    // Mining to the left and right
    for (let x = botPosition.x - 1; x <= botPosition.x + 1; x += 2) {
        for (let y = startY - 10; y <= startY - 1; y++) {
            await bot.dig(bot.blockAt(new Vec3(x, y, botPosition.z))); if(bot.interrupt_code) {log(bot, "Code interrupted.");return;}
        }
    }
