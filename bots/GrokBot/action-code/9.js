    ...
    for (let z = 0; z < hallwayLength; z++) {
        for (let x = 0; x < hallwayWidth; x++) {
            for (let y = 0; y < hallwayHeight; y++) {
                await skills.digBlock(bot, new Vec3(startPos.x + x, startPos.y + y, startPos.z + z)); if(bot.interrupt_code) {log(bot, "Code interrupted.");return;}
            }
        }
    }
    ...
    for (let z = 0; z < roomLength; z++) {
        for (let x = 0; x < roomWidth; x++) {
            for (let y = 0; y < roomHeight; y++) {
                await skills.digBlock(bot, new Vec3(roomStartPos.x + x, roomStartPos.y + y, roomStartPos.z + z)); if(bot.interrupt_code) {log(bot, "Code interrupted.");return;}
            }
        }
    }
